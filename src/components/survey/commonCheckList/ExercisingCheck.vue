    ...
    <form style="padding-inline: 30%;">
      <div>
        <div style="font-size: xx-large;">
          <div style="margin-top:10%; height:100%; width:100%; display:flex; justify-content:center; align-items:center; font-weight: 100;">
            <p style="margin-top:3%; font-weight: bold; ">야외활동</p>을 많이 하는 편인가요?
          </div>
        </div>
      </div>
      <div class="form-radio" style=" margin-top:8%;">
        <ul class="ul">
          <li>
            <input type="radio" name="exercising" id="true" value="true" v-model="exercising" @click="exercisingCheck" checked="checked">
            <label class="form-check-label" for="true">네</label>
          </li>
          <li>
            <input type="radio" name="exercising" id="false" value="false" v-model="exercising" @click="exercisingCheck">
            <label class="form-check-label" for="false">아니요</label>
          </li>
        </ul>
      </div>
      <div v-if="exercising == 'true'">
        <div class="form-group" style="margin-top:10%; margin-left:15%; width:80%;">
          <label>야외활동 횟수</label>
            <div class="form-check" style="margin-top:5%;">
              <input class="form-check-input" id="exercisingAmount1" type="radio" value="주 1~2회" v-model="exercisingAmount" @click="exercisingAmountCheck">
              <label class="form-check-label" for="exercisingAmount1">주 1~2회</label>
            </div>
            <div class="form-check">
              <input class="form-check-input" id="exercisingAmount2" type="radio" value="주 3~4회" v-model="exercisingAmount" @click="exercisingAmountCheck">
              <label class="form-check-label" for="exercisingAmount2">주 3~4회</label>
            </div>
            <div class="form-check">
              <input class="form-check-input" id="exercisingAmount3" type="radio" value="주 5회이상" v-model="exercisingAmount" @click="exercisingAmountCheck">
    ...
    <div style="display:flex; justify-content:center; align-items:center; width:100%; height:100px; padding-inline: 25%; text-align: center;">
      <div style="width:100%; display:flex; padding-inline: 20%;">
        <div style="width:40%; display:flex; ">
          <router-link :to="{ name: 'DrinkingCheckPage' }" type="submit" class="btn btn-primary" 
...
export default {
  ...
  mounted() {
    ...
    if(commonSurveyResult != null) {
      ...
      let exercising = this.commonSurveyResult.exercising;
      if(!(exercising == null || exercising == '' || exercising == undefined)) {
        this.next.exercising = true;
        this.exercising = exercising;
      }
      
      let exercisingAmount = this.commonSurveyResult.exercisingAmount;
      if(!(exercisingAmount == null || exercisingAmount == '' || exercisingAmount == undefined)) {
        this.next.exercisingAmount = true;
        this.exercisingAmount = exercisingAmount;
      }
    }
    ...
    this.beforeCheck(this.commonSurveyResult.height, "DrinkingCheckPage", "음주 여부가 제대로 체크 되지 않았습니다.\음주 여부 체크 페이지로 이동합니다.");
  },
  methods: {
    ...
    exercisingAmountCheck() {
      this.next.exercisingAmount = true;
    },
    nextPage() {
      if(this.next.exercising == false) {
        alert("야외활동 여부를 체크해주세요.");
        return;
      }
      if(this.exercising == "true" && this.next.exercisingAmount == false) {
        alert("야외활동량을 체크해주세요.");
        return;
      }

      this.commonSurveyResult.exercising = this.exercising;
      if(this.exercising == "false") this.commonSurveyResult.exercisingAmount = "";
      else this.commonSurveyResult.exercisingAmount = this.exercisingAmount;
      localStorage.setItem('commonSurveyResult', JSON.stringify(this.commonSurveyResult));
      this.$router.push({ name: "VisceraSurveyPage" });
    }
  }
}
