        ...
        <div v-else>
            <v-card v-for="wish in wishList" :key="wish.wishId" class="mb-5 rounded-xl">
                <v-row>
                    ...
                    <v-col cols="2">
                        <router-link :to="{ name: 'DetailProductPage', params: { productId: wish.productId } }" class="baby-product-link">
                            <v-img :src="require(`@/assets/productImgs/${wish.thumbnail}`)" 
                                cover class="grey lighten-2" style="border-radius: 20px;"/>
                        </router-link>
                    </v-col>
                    <v-col>
                        <router-link :to="{ name: 'DetailProductPage', params: { productId: wish.productId } }"
